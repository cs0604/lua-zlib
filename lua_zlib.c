...
static int lz_deflate_new(lua_State *L) {
    ...
    stream->zfree  = Z_NULL;
    lz_assert(L, deflateInit2(stream, level,Z_DEFLATED,-MAX_WBITS, MAX_MEM_LEVEL, Z_DEFAULT_STRATEGY), stream, __FILE__, __LINE__);
//&stream, level, 8, 15, 8, 0, zlibVersion(), sizeof(z_stream)
//strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED,windowBits | GZIP_ENCODING, 8,Z_DEFAULT_STRATEGY
